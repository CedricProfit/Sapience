...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Sapience is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Sapience is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Sapience is not connected!");

   // if (IsInitialBlockDownload())
   //     throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Sapience is downloading blocks...");
